...
class Model(object):
    ...
    def __init__(self, n_node, dim_node, dim_edge, dim_h=50, n_mpnn_step=5, dr=0.1, batch_size=20):
        ...
        self.Y = tf.placeholder(tf.float32, [self.batch_size, self.maxlen])
    ...
    def train(self, DV, DE, DY, DM, save_path, frac_val = 0.05):
        ...
        def list_to_set(y):
            set = np.sort(y) #invariant to permutation of atoms
            set = np.pad(set, (0, self.maxlen - len(y)), 'constant', constant_values = self.const) 
            
            return set

        DV_trn, DV_val, DE_trn, DE_val, DY_trn, DY_val, DM_trn, DM_val = train_test_split(DV, DE, DY, DM, test_size = frac_val)   
        DY_trn = np.array([list_to_set(y) for y in DY_trn])

        ## objective function
        reg = tf.square(tf.concat([tf.reshape(v, [-1]) for v in tf.trainable_variables()], 0))
        l2_loss = 1e-10 * tf.reduce_mean(reg)
        
        calib = np.std(DY_trn.flatten()[DY_trn.flatten()<self.const])

        def tf_transform(i):
            a = tf.repeat(tf.squeeze(self.Y_pred[i]), repeats = tf.squeeze(tf.dtypes.cast(self.Y_mask[i], tf.int32)), axis=0)
            a = tf.sort(a, axis=0)
            a = tf.pad(a, [[0, self.maxlen - tf.reduce_sum(self.Y_mask[i])]], mode='CONSTANT', constant_values=self.const)
            
            return a

        Y_pred_sort = tf.stack([tf_transform(i) for i in range(self.batch_size)])
        cost_Y = tf.reduce_sum( tf.abs((self.Y - Y_pred_sort) / calib) ) / tf.reduce_sum(self.Y_mask) #invariant to permutation of atoms
    ...
    def val_mae(self, DV, DE, DY, DM, m):
        ...
        abs_err = []            
        for i, dm in enumerate(DM):
            predvals = []
            for j in range(len(dm)):
                for _ in range(dm[j,0]): predvals.append(DY_hat[i,j,0])

            assert DY[i].shape == np.sort(predvals).shape
            abs_err = abs_err + np.abs(DY[i] - np.sort(predvals)).tolist() #invariant to permutation of atoms
